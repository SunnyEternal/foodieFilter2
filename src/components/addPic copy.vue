  ...
  <div class="container">
    ...
    <section>
      ...
      <div class="relatedTopic">
        ...
        <ul class="topicItem">
          <li @click="selectedTopicFn(index)" v-for="(item, index) in topic3List" :key="item.topicId" :class="{sel: currTopic === index}">{{item.topicName}}</li>
        </ul>
...
export default {
  ...
  this.$loading(1)
      ...
      for (let file of this.files) {
        ...
        //   url: 'http://foodie-beta.kajicam.com/fs/api/pic/upload',
        //   method: 'post',
        //   config: config
        //   // processData: false,
        //   // contentType: false,
  ...
  this.$router.push({name: 'list'})
      // if (this.isUploading) {
      //   // this.$toast('请等待照片先上传完成！', 1000)
      // } else if (this.tempFilePaths.length) {
      //   this.$parent.imgList = this.list
      //   this.goToPage('Play',['_trackEvent', 'musicAlbum', 'btn', `提交按钮照片${this.list.length}张的点击量`])
      // } else {
      //   // this.$toast('请先上传照片！')
      // }
    },
